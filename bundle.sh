...
uglifyjs --compress warnings=false --mangle -- "$tmpbundle" "$DEV_FILE" >> "$PROD_FILE"
...
rm -rf "$DIST/ajax1.txt" && echo "Hello from AJAX1!" > "$DIST/ajax1.txt"
rm -rf "$DIST/ajax2.txt" && echo "Hello from AJAX2!" > "$DIST/ajax2.txt"
